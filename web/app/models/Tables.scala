...
trait Tables {
   ...
   *  @param `class` Database column class DBType(varchar), Length(2147483647,true)
   *  @param rate Database column rate DBType(int4), Default(None) */
  case class TutorsRow(user: String, `class`: String, rate: Option[Int] = None)
  ...
  class Tutors(_tableTag: Tag) extends Table[TutorsRow](_tableTag, "tutors") {
    ...
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (user.?, `class`.?, rate).shaped.<>({r=>import r._; _1.map(_=> TutorsRow.tupled((_1.get, _2.get, _3)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    /** Database column class DBType(varchar), Length(2147483647,true)
     *  NOTE: The name was escaped because it collided with a Scala keyword. */
    val `class`: Column[String] = column[String]("class", O.Length(2147483647,varying=true))
