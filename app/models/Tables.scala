...
trait Tables {
  ...
  case class CoursesRow(id: String, major: String, name: String, description: Option[String] = None, offered: Option[String] = None, planLink: Option[String] = None, prereqs: Option[String] = None)
  ...
  implicit def GetResultCoursesRow(implicit e0: GR[String], e1: GR[Option[String]]): GR[CoursesRow] = GR{
    prs => import prs._
    CoursesRow.tupled((<<[String], <<[String], <<[String], <<?[String], <<?[String], <<?[String], <<?[String]))
  }
  ...
  class Courses(_tableTag: Tag) extends Table[CoursesRow](_tableTag, "courses") {
    def * = (id, major, name, description, offered, planLink, prereqs) <> (CoursesRow.tupled, CoursesRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (id.?, major.?, name.?, description, offered, planLink, prereqs).shaped.<>({r=>import r._; _1.map(_=> CoursesRow.tupled((_1.get, _2.get, _3.get, _4, _5, _6, _7)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    lazy val majorsFk = foreignKey("courses_major_fkey", major, Majors)(r => r.id, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.NoAction)
  }
  ...
  case class MajorsRow(id: String, college: String, name: String, link: Option[String] = None)
  ...
  implicit def GetResultMajorsRow(implicit e0: GR[String], e1: GR[Option[String]]): GR[MajorsRow] = GR{
    prs => import prs._
    MajorsRow.tupled((<<[String], <<[String], <<[String], <<?[String]))
  }
  ...
  class Majors(_tableTag: Tag) extends Table[MajorsRow](_tableTag, "majors") {
    def * = (id, college, name, link) <> (MajorsRow.tupled, MajorsRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (id.?, college.?, name.?, link).shaped.<>({r=>import r._; _1.map(_=> MajorsRow.tupled((_1.get, _2.get, _3.get, _4)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    lazy val collegesFk = foreignKey("majors_college_fkey", college, Colleges)(r => r.id, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.NoAction)
  }
   ...
   *  @param user Database column user DBType(varchar), Length(2147483647,true), Default(None)
   *  @param `class` Database column class DBType(varchar), Length(2147483647,true), Default(None)
   *  @param rate Database column rate DBType(int4), Default(None) */
  case class TutorsRow(user: Option[String] = None, `class`: Option[String] = None, rate: Option[Int] = None)
  /** GetResult implicit for fetching TutorsRow objects using plain SQL queries */
  implicit def GetResultTutorsRow(implicit e0: GR[Option[String]], e1: GR[Option[Int]]): GR[TutorsRow] = GR{
    prs => import prs._
    TutorsRow.tupled((<<?[String], <<?[String], <<?[Int]))
  }
  ...
  class Tutors(_tableTag: Tag) extends Table[TutorsRow](_tableTag, "tutors") {
    def * = (user, `class`, rate) <> (TutorsRow.tupled, TutorsRow.unapply)
    
    /** Database column user DBType(varchar), Length(2147483647,true), Default(None) */
    val user: Column[Option[String]] = column[Option[String]]("user", O.Length(2147483647,varying=true), O.Default(None))
    /** Database column class DBType(varchar), Length(2147483647,true), Default(None)
     *  NOTE: The name was escaped because it collided with a Scala keyword. */
    val `class`: Column[Option[String]] = column[Option[String]]("class", O.Length(2147483647,varying=true), O.Default(None))
    /** Database column rate DBType(int4), Default(None) */
    val rate: Column[Option[Int]] = column[Option[Int]]("rate", O.Default(None))
    
    /** Foreign key referencing Courses (database name tutors_class_fkey) */
    lazy val coursesFk = foreignKey("tutors_class_fkey", `class`, Courses)(r => r.id, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.NoAction)
   ...
   *  @param firstName Database column first_name DBType(varchar), Length(2147483647,true)
   *  @param lastName Database column last_name DBType(varchar), Length(2147483647,true)
  ...
  case class UsersRow(firstName: String, lastName: String, email: String, password: String, salt: String, verified: Option[Boolean] = None, student: Option[Boolean] = None, tutor: Option[Boolean] = None, rate: Option[Int] = None, about: Option[String] = None)
  /** GetResult implicit for fetching UsersRow objects using plain SQL queries */
  implicit def GetResultUsersRow(implicit e0: GR[String], e1: GR[Option[Boolean]], e2: GR[Option[Int]], e3: GR[Option[String]]): GR[UsersRow] = GR{
    prs => import prs._
  ...
  class Users(_tableTag: Tag) extends Table[UsersRow](_tableTag, "users") {
    ...
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (firstName.?, lastName.?, email.?, password.?, salt.?, verified, student, tutor, rate, about).shaped.<>({r=>import r._; _1.map(_=> UsersRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6, _7, _8, _9, _10)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    
    /** Database column first_name DBType(varchar), Length(2147483647,true) */
    val firstName: Column[String] = column[String]("first_name", O.Length(2147483647,varying=true))
    /** Database column last_name DBType(varchar), Length(2147483647,true) */
    val lastName: Column[String] = column[String]("last_name", O.Length(2147483647,varying=true))
